...
'''
    ...
    def cli(*args):
        result = subprocess.run([str(ROOT/'target/debug/wallfolio'), *args], env=env, capture_output=True, text=True, timeout=20)
    ...
    try:
        ...
        cli('rotation', 'start', '--interval', '10')
        log.write_text('')
        ...
        daemon.wait(timeout=5)
        start()
        assert cli('rotation', 'status')['enabled']
        cli('rotation', 'stop')
        assert not cli('rotation', 'status')['enabled']
        assert cli('random')['applied']
        other = tmp/'identical.png'
        other.write_bytes(source.read_bytes())
        duplicate = cli('add', str(other))
        cli('download', duplicate['id'])
        groups = cli('duplicates')
        assert groups[0]['count'] == 2 and len(groups[0]['items']) == 2
